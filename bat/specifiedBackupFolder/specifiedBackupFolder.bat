...
REM pref.iniから情報の取得
REM set INIFILE=%~dp0pref.ini
set INIFILE=%~dp0%~n0.ini
call :GET_INIFILE_INFO "PATH" "backuppath" OUTPUTPATH !INIFILE!
...
set INDEX_COUNT=1
REM バックアップ先のフォルダ内アイテムがあった場合ネーム情報を調査し
REM 同一ネームフォルダが合った場合インデックスを付与する
if exist !BACKUP_OUTPUTTOPPATH!\ (
    for /f "usebackq" %%a in (`dir /b !BACKUP_OUTPUTTOPPATH!`) do (
        echo "%%a" | findstr "!BACKUP_INPUTNAME!_!NOW_DATE!">NUL
        if not ERRORLEVEL 1 (
            set /a INDEX_COUNT+=1
        )
    )
